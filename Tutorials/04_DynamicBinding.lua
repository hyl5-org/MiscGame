...
-- use unlua bindkey interface

local BindKey = UnLua.Input.BindKey

-- local testClass = require("Tutorials.testClass")

BindKey(M, "One", "Pressed", function(self, Key)
    Screen.Print("1 Pressed")

    local World = self:GetWorld()
    local Transform = self.SpawnPointActor:GetTransform()
    local AlwaysSpawn = UE.ESpawnActorCollisionHandlingMethod.AlwaysSpawn
    local testActor = UE.UClass.Load("/Game/Core/Blueprints/TestActor.TestActor_C")
    -- testActor = UE.UClass.Load("/Game/Core/Blueprints/Weapon/BP_DefaultProjectile.BP_DefaultProjectile_C")
